...
public class JobPostingParseService {
	...
	private String createJobParsingPrompt(String content, String positionList) {
		...
		[파싱 규칙]:
		1. **입력 패턴**: 각 줄은 "✓ 회사명 / 직무명 / 마감일 (URL)" 형식일 수 있습니다. 이 형식을 기준으로 회사명, 직무, 마감일, URL을 정확히 추출하세요.
		2. **정확성**: 회사명, 직무명 원문, 마감일(MM/DD 형식), URL을 누락 없이 정확히 추출해야 합니다.
		3. **특수문자 보존**: URL의 특수문자(http, :, /, ?, =, &)는 절대 변경하거나 제거하지 마세요. 직무명(예: Node.js)이나 회사명(예: S.S.G)에 포함된 특수문자도 그대로 보존해야 합니다.
		4. **불필요한 내용 무시**: 빈 줄, 헤더(예: '11월 11일 채용 공고'), 인사말 등 공고 목록과 직접 관련 없는 텍스트는 무시하세요.
